...
class GenConfig:
    ...
    temperature: float = 0.0          # 0.0 => greedy / reproducible
    top_p: float = 0.95
...
class LLM(ABC):
    ...
    @abstractmethod
    def generate(self, messages: list[Message], config: GenConfig | None = None) -> str:
        ...

    def stream(self, messages: list[Message], config: GenConfig | None = None) -> Iterator[str]:
...
class TransformersLLM(LLM):
    ...
    def _encode(self, messages: list[Message]):
        try:
            ...
            return self.tok.apply_chat_template(
                _merge_system_into_user(messages), add_generation_prompt=True,
                return_tensors="pt", return_dict=True,
            ).to(self.device)
    ...
    def _gen_kwargs(self, cfg: GenConfig) -> dict:
        ...
        kw = dict(max_new_tokens=cfg.max_new_tokens, pad_token_id=pad, do_sample=cfg.do_sample)
...
class LlamaCppLLM(LLM):
    def __init__(self, *, repo_id: str | None = None, filename: str | None = None,
                 model_path: str | None = None, n_ctx: int = 8192, n_gpu_layers: int = -1):
        ...
        if model_path:
            ...
            self.llm = Llama.from_pretrained(repo_id=repo_id, filename=filename, **common)

    def generate(self, messages, config=None):
        cfg = config or GenConfig()
        out = self.llm.create_chat_completion(
            messages=messages, max_tokens=cfg.max_new_tokens,
            temperature=cfg.temperature, top_p=cfg.top_p, stop=cfg.stop or None,
        )
    ...
    def stream(self, messages, config=None):
        ...
        for chunk in self.llm.create_chat_completion(
            messages=messages, max_tokens=cfg.max_new_tokens,
            temperature=cfg.temperature, top_p=cfg.top_p, stop=cfg.stop or None, stream=True,
        ):
...
REGISTRY: dict[str, Spec] = {
    ...
    "gemma-2b": Spec("transformers", {"model_id": "google/gemma-2-2b-it"}),
